    ...
    $.ajax({
        ...
        success: function (msg) {
            ...
            if (msg.SDictionaryDetails.length < 1) {
                ...
                $(window.parent.document).find(":div#popupContact h1").html("编辑选项");
            } else {
                ...
                str += "<br/><input type='text' value='' class='newvalue'/>";
                $(window.parent.document).find(":div#contactArea").html(str);
                $(window.parent.document).find(":div#popupContact h1").html("编辑--" + msg.SDictionaryDetails[0].SDictionaryClass.name);
            }
...
function updateValue(objname) {
    var objs = $(window.parent.document).find(":div#contactArea .oldvalue");
    var addObj = $(window.parent.document).find(":div#contactArea .newvalue").val();
    ...
    $.ajax({
        ...
        success: function (msg) {
            loadPopup(objname);
            getNewVal(objname);
            $(window.parent.document).find(":div#dictionaryMsg").html("修改成功！");
        }
    });
...
function upsort(sort, objname) {
    ...
    $.ajax({
        ...
        success: function (msg) {
            loadPopup(objname);
            getNewVal(objname);
            $(window.parent.document).find(":div#dictionaryMsg").html("向上移动成功！");
        }
    });
}
function downsort(sort, objname) {
    ...
    $.ajax({
        ...
        success: function (msg) {
            loadPopup(objname);
            getNewVal(objname);
            $(window.parent.document).find(":div#dictionaryMsg").html("向下移动成功！");
        }
    });
}
function del(_id, objname) {
    ...
    $.ajax({
        ...
        success: function (msg) {
            loadPopup(objname);
            getNewVal(objname);
            $(window.parent.document).find(":div#dictionaryMsg").html("删除成功！");
        }
    });
}
