...
function loadPopup4nextTDate(objname) {
    ...
    $(window.parent.document).find(":div#contactArea").html("<iframe style='width:100%;height:100%;' name='nextTDatePage' src='jsp/company/cal.jsp' frameborder='0'></iframe>");
...
function disablePopup4nextTDate() {
    if (popupStatus == 1) {
        ...
        getNextTDate();
        //dz
        $(window.parent.document).find(":div#backgroundPopup").html("");
        $(window.parent.document).find(":div#popupContact").attr("style", "");
        $(window.parent.document).find(":div#backgroundPopup").attr("style", "");
    }
}
